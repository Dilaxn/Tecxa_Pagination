...
function ProductList() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const url =  `${BASE_URL}/products?limit=${limit}&skip=${(page - 1) * limit}&category=${_category ? _category : ""}`
    ...
    const handleChange = async (event, value) => {
        setPage(value);
        try {
            navigate(`/${_category ? _category : ""}?page=${value}&limit=${limit}&skip=${(value - 1) * limit}`)
        } catch (error) {
            console.error(error);
        }
    };
    ...
    return (<>{!isLoading1 && !isLoading2 ?
            <Grid container>
                <Grid xs={9}><h1
                    style={{marginLeft: 10}}>{_category && _category !== "all products" ? _category : "Products"}</h1>
                </Grid>
                <Grid xs={3}><Box mb={2}>
                    <FormControl variant="outlined" style={{width: "90%", minWidth: 200, marginTop: 20}}>
                ...
                <Grid container style={{justifyContent: "center", alignItems: "center", marginBottom: 20, marginTop:20}}>
                    <Grid item>
                        <Stack spacing={2}
